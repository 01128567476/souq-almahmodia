  ...
  providers: [
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        if (!user.role) {
          console.error("[AUTH] role is null or undefined for credentials login!");
  ...
  callbacks: {
     ...
     */
      async jwt({ token, user, trigger }: any) {
          ...
          if (user.role) {
            ...
            try {
              ...
              if (freshUser.length > 0 && freshUser[0].role) {
                ...
                console.error("[AUTH] jwt: user exists but role not found in DB");
                token.role = "user" as Role;
              }
            } catch (error) {
              console.error("[AUTH] jwt: failed to fetch role from DB:", error);
              token.role = "user" as Role;
            }
          } else {
            console.error("[AUTH] jwt: no user.id to fetch role");
            token.role = "user" as Role;
          }
        }
        ...
        if (!token.role) {
          console.error("[AUTH] jwt: token.role still missing after all attempts");
          token.role = "user" as Role;
        }
